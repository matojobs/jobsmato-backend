...
This file tracks deployment activities. For general dev activity (fixes, tooling, rules), see [ACTIVITY_LOG.md](./ACTIVITY_LOG.md).
...
### Changes Deployed
- **CORS:** Added recruiter portal prod origin so HRMS can call the backend on production.
- **Allowed origin:** `https://hrms.jobsmato.com` (Recruiter/HRMS frontend at [HRMS Jobsmato](https://hrms.jobsmato.com/)).

### Files Modified
- `src/main.ts` – added `'https://hrms.jobsmato.com'` to `allowedOrigins` in CORS config.

### Deployment Steps
1. Add HRMS origin to CORS in `main.ts`.
2. Build and deploy backend via `deploy.ps1` (or existing process).
3. Verify: from HRMS, calls to `https://api.jobsmato.com` (or prod API URL) succeed and no CORS errors in browser.

### Notes
- Recruiter portal at https://hrms.jobsmato.com/ can now call the backend API from the browser (same-origin policy satisfied).
- No migration. No env change required for CORS (origin list is in code).

---

## 2026-02-05 - Create first admin on production

### Script added
- **scripts/create-admin-user.js** – Creates or upgrades a user to admin using the same DB env as the app (DATABASE_URL or DB_*).

### How to create an admin on prod
1. Set prod DB credentials (e.g. copy prod `.env` or set `DATABASE_URL` / `DB_HOST`, `DB_USERNAME`, `DB_PASSWORD`, `DB_NAME`).
2. From repo root run:
   ```bash
   node scripts/create-admin-user.js --email admin@jobsmato.com --password "YourSecurePassword123!"
   ```
3. Optional: `--first "Admin"` and `--last "User"` for display name.
4. Log in to the admin panel with that email and password.

### Notes
- If the email already exists, the user is updated to role `admin` and the password is set.
- Password must be at least 8 characters.

---

## 2026-02-05 - Google OAuth Fixes (Pre-Deploy)

### Changes (deploy with next release)
- **Trust proxy** in `main.ts` for correct redirect URLs behind Nginx/Cloudflare.
- **Google strategy:** Require `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and full `GOOGLE_CALLBACK_URL`; safe profile validation; no dummy credentials.
- **Auth callback:** Handle missing user and login errors; redirect to frontend with `?error=google_auth_failed&message=...`; normalize `FRONTEND_URL`.
- **Docs:** `env.example` updated with Google OAuth setup notes.

### Files Modified
- `src/main.ts`, `src/modules/auth/strategies/google.strategy.ts`, `src/modules/auth/auth.controller.ts`, `env.example`

### Deployment Notes
- No migration. Ensure `GOOGLE_CALLBACK_URL` is set to full URL in production (e.g. `https://api.jobsmato.com/api/auth/google/callback`). Google Console redirect URI must match exactly. See [ACTIVITY_LOG.md](./ACTIVITY_LOG.md) for details.
